...
const Register = () => {
  ...
  // Apply and save dark mode changes
  useEffect(() => {
    if (isDarkMode) {
      document.documentElement.classList.add("dark");
    } else {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch("https://securenest-t72z.onrender.com/register", {
        method: "POST",
      ...
      if (res.ok) {
        showPopup(data.message || "Registration successful", "success");
    ...
    >
      ...
      <form
        ...
        className={`w-full max-w-md p-10 backdrop-blur-lg rounded-3xl shadow-xl border transition-colors duration-500 ${
          isDarkMode ? "bg-white/10 border-gray-700" : "bg-white/90 border-gray-200"
        }`}
      >
        ...
        <div className="mb-5">
          <label htmlFor="username" className={`block mb-2 text-sm font-semibold ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}>
            Username
          </label>
          <input
            type="text"
        ...
        <div className="mb-5">
          <label htmlFor="email" className={`block mb-2 text-sm font-semibold ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}>
            Email
          </label>
          <input
            type="email"
        ...
        <div className="mb-8">
          <label htmlFor="password" className={`block mb-2 text-sm font-semibold ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}>
            Password
          </label>
            ...
            >
              {showPassword ? (
                ...
                <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.686 5 12 5s8.268 2.943 9.542 7c-1.274 4.057-5.228 7-9.542 7s-8.268-2.943-9.542-7z" />
                </svg>
              )}
            </button>
          </div>
